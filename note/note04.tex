  ...
  \begin{columns}
    ...
    \begin{column}{0.45\textwidth}
      ...
      \begin{tikzpicture}[scale=0.85]
        \draw[->] (0,0) -- (4.5,0) node[right, font=\scriptsize] {$p$};
        \draw[->] (0,0) -- (0,4.5) node[above, font=\scriptsize] {$\pi(p)$};
        
        % Linear probability
        \draw[domain=0:4, dashed] plot (\x, \x) node[right, font=\scriptsize] {$\pi(p) = p$};
        
        % Weighting function
        \draw[domain=0.01:4, blue, thick, samples=100] plot (\x, {(\x^0.65)/((\x^0.65) + (4-\x)^0.65)^(1/0.65)});
        ...
        % Grid lines
        \foreach \x in {1,...,4}
        \draw (\x,0.1) -- (\x,-0.1) node[below, font=\scriptsize] {\x/4};
        \foreach \y in {1,...,4}
        \draw (0.1,\y) -- (-0.1,\y) node[left, font=\scriptsize] {\y/4};
        
        % Annotations
        \draw[->, red] (0.7,.8) -- (0.3,1.3) node[above, font=\scriptsize] {Overweighting};
        \draw[->, red] (2.6,2.5) -- (3,2) node[right, font=\scriptsize] {Underweighting};
      \end{tikzpicture}
    \end{column}
  \end{columns}
\end{frame}
