...
\begin{frame}{Problem 3 Solution}
  ...
  \end{align*}
  which is true only when $X$ is normally distributed. For the final part
  \begin{align*}
    \psi''\psi - (\psi')^2 = \expc{X^2e^{-aX}}\expc{e^{-aX}} - (\expc{Xe^{-aX}})^2 \geqslant 0
  \end{align*}
  by the Cauchy-Schwarz inequality applied to r.v.s $A = Xe^{-\frac{a}{2}X}$ and $B = e^{-\frac{a}{2}X}$. To see that $\alpha$ is increasing
  \begin{align*}
...
Write $X = \mu + \sigma Y$ where $Y\sim N(0,1)$. Then it follows that
  ...
  \end{align*}
  by the concavity of $v$. Now when returns are normally distributed then the wealth created by each portfolio is normally distributed; this argument shows that maximizing in $\sigma$ for fixed $\mu$ gives a value of $(\sigma,\mu)$ on the efficient frontier. To see the concavity of $f$, note that
  \begin{align*}
